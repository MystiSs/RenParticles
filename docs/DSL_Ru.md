...
* [Оптимизация производительности](#оптимизация-производительности-системы-частиц)
    * [Оптимизация трансформаций](#оптимизация-трансформаций)
    * [Оптимизация логики обновления](#оптимизация-логики-обновления)
    * [Точность обновления](#точность-обновления)
    * [Целевой FPS](#целевой-fps-для-адаптивных-оптимизаций)
    * [Передача событий каждой частице от системы](#передача-событий-каждой-частице-от-системы)
...
* [Эмиттеры](#эмиттеры)
---
* [Эмиттер: spray](#эмиттер-spray)
* [Эмиттеры: radial_spray / ring_spray](#эмиттеры-radial_spray--ring_spray)
* [Эмиттер: radial_spray](#эмиттер-radial-spray)
* [Эмиттер: ring_spray](#эмиттер-ring-spray)
* [Эмиттер: interval_spray](#эмиттер-interval_spray)
* [Эмиттеры: radial_interval_spray / ring_interval_spray](#эмиттеры-radial_interval_spray--ring_interval_spray)
...
* [Поведения частиц](#поведения-частиц-обработчики)
    ...
    * [Условная инструкция if](#условная-инструкция-if)
---
...
* [Обработчик: spring](#обработчик-spring)
* [Обработчик: wander](#обработчик-wander)
* [Обработчик: rotate](#обработчик-rotate)
...
* [Обработчики зон событий: event_zone и его вариации](#обработчики-зон-событий)
* [Обработчик: mouse_event](#обработчик-mouse_event)
* [Обработчик: sound](#обработчик-sound)
---
* [Пресеты](#пресеты)
...
* [Модели (шаблоны)](#модели-шаблоны)
    ...
    * [Переопределение свойств обработчиков](#переопределение-свойств-обработчиков)
* [Управление системами](#управление-системами)
    * [Команды управления](#команды-управления)
* [Принудительная симуляция](#принудительная-симуляция)
...
Движок RenParticles поддерживает несколько систем оптимизации. Они позволяет поддерживать высокую частоту кадров (FPS) даже при отображении большого (в рамках движка RenPy) количества частиц, автоматически распределяя вычислительную нагрузку.

### Оптимизация трансформаций

```renpy
accelerate transforming        # Режим включён. Если accelerate transforming не писать, то он будет соответственно отключён 
```

Если нагрузка на систему возрастает, движок автоматически распределяет вычисления трансформаций на несколько кадров. Это защищает игру от "просадок" FPS в тяжелых системах частиц.


- Данная инструкция полезна только для систем частиц, которые используют `Transform` обработчки;
- Ценой оптимизации является снижение плавности анимации трансформаций. Максимальное количество "разделений" обработок – каждый 16-й кадр. Если движок решит работать в таком режиме, это будет заметно;
- Может работать в связке с другими включёнными оптимизациями;
- Инструкция не имеет параметров: достаточно просто добавить строку accelerate transforming в описание системы частиц.

---

### Оптимизация логики обновления

```renpy
accelerate update        # Режим включён. Если accelerate update не писать, то он будет соответственно отключён 
```

Включает автоматический буфер для логики обновлений (on update). Позволяет «растягивать» вычисление поведения частиц при падении производительности.

- Рекомендуется для сложных систем с большим количеством скриптовой логики в on update блоке и большим количеством частиц;
- Ценой оптимизации является снижение плавности обработки. Максимальное количество "разделений" обработок – каждый 16-й кадр. Если движок решит работать в таком режиме, это будет **очень** заметно. Уже начиная с оптимизации обновлять логику каждый 4-й кадр, будет заметно;
- Может работать в связке с другими включёнными оптимизациями;
- Инструкция не имеет параметров: достаточно просто добавить строку accelerate update в описание системы частиц.

---

### Целевой FPS для адаптивных оптимизаций

```renpy
acceleration target fps 45     # accelerate update и accelerate transforming будут стремиться поддерживать 45 кадров с секунду
```

Устанавливает целевое значение FPS, к которому будет стремиться система оптимизации (по умолчанию 60).
...
```renpy
update fidelity 2       # Обновление происходит каждый 2-й кадр
```

Задает базовый уровень точности обновления. Значение 1 означает обновление каждый кадр. Значение 2 — обновление через кадр и т.д.

Это «жесткое» ограничение. Если вы установили update `update fidelity 2`, система будет обновлять частицы в 2 раза реже независимо от текущей нагрузки. Это самый простой способ снизить нагрузку на CPU.

- Требует параметр в виде *целого числа*;
- Рекомендуется для сложных систем с большим количеством скриптовой логики в on update блоке и большим количеством частиц;
- Ценой оптимизации является снижение плавности обработки.
- Может работать в связке с другими включёнными оптимизациями.

---

### Передача событий каждой частице от системы

```renpy
particles listening events        # Передача событий включена. Если particles listening events не писать, то передача будет соответственно отключена
```

Включает передачу событий, которая получает система частиц самим частицам. Например, событие движение мыши будет передано всем текущим живым частицам. Полезно, если у вас изображения для частиц это сложные пользовательские `CDD` или подобные классы.

- Включение этой опции может **значительно** увеличить нагрузку на CPU, если частиц в системе огромное количество (300+). Это будет заметно, когда вы начнёте водить мышью.
...
Реагирует на различные `pygame` события:
...
system id "magical_spark":
    ...
    # Блок инициализации при рождении частицы
    on particle appear:
        # Ваши обработчики и модификаторы
        sound:
            file "sfx/spawn.ogg"
            volume 0.4
```
...
### Эмиттеры `radial_spray` / `ring_spray`

Генерируют заданное количество частиц **мгновенно** (в один кадр), но область генерации в полярных координатах.
* `spray_radial`: Заполняет **всю площадь** круга.
* `spray_ring`: Создает частицы только **по периметру** (с учетом `width`).

**Параметры:**

* **`amount`** (целое число) — Количество создаваемых частиц.
* **`radius`** (целое число) — Радиус круга или кольца. По умолчанию `100`.
* **`center`** (2 числа/строка) — Координаты центра `(x, y)` или строка `"mouse"`. Если задан как `"mouse"`, то используются координаты позиции мыши.
* **`width`** (целое число) — *(Только для `ring_spray`)* Ширина «стенки» кольца. Если `0`, частицы появятся строго на линии окружности.
...
### Эмиттеры `radial_interval_spray` / `ring_interval_spray`

Генерируют частицы **постепенно** с заданным интервалом, но область генерации в полярных координатах.
* `radial_interval_spray`: Заполняет **всю площадь** круга.
* `ring_interval_spray`: Создает частицы только **по периметру** (с учетом `width`).

**Параметры:**

* **`interval`** (число) — Пауза между спавном в секундах. По умолчанию `0.0` (каждый кадр).
* **`per_amount`** (целое число) — Сколько частиц создавать за один такт интервала.
* **`amount`** (целое число/строка) — Общее количество частиц. Можно указать `"infinite"` для бесконечного потока.
* **`kill_on_finish`** (логическое значение) — Удалять ли эмиттер после того, как лимит `amount` исчерпан.
* **`radius`** (целое число) — Радиус круга или кольца. По умолчанию `100`.
* **`center`** (2 числа/строка) — Координаты центра `(x, y)` или строка `"mouse"`. Если задан как `"mouse"`, то используются координаты позиции мыши.
* **`width`** (целое число) — *(Только для `ring_interval_spray`)* Ширина «стенки» кольца. Если `0`, частицы появятся строго на линии окружности.
...
on update:
    emitter interval_spray_ring:
        ...
        radius 200
        width 20
        center "mouse" # Центр – позиция курсора
...
*RenParticles* поддерживает условное выполнение обработчиков и пресетов с помощью конструкции `if`. Это позволяет управлять поведением частиц в зависимости от различных условий.

```renpy
...

on update:
    обработчик if "условие":
        ...
```
...
preset my_preset if "renpy.mouse_get_pos() > (240, 150)" # Условие будет передано всем обработчикам, которые есть в пресете

```

**Особенности:**

* Условие указывается в кавычках как `строка` Python-выражения.
* Выражение вычисляется **для каждой частицы** при каждом вызове обработчика.
* Если условие истинно (`True`), обработчик выполняется. Если ложно — пропускается.


**Примеры:**

#### 1. Случайное поведение
```renpy
on update:
...
Пружинистое притяжение к точке или мыши с инерцией, затуханием и физикой пружины. Создаёт естественное "резиновое" движение частиц.
...
on update:
    spring:
        target "mouse"          # или, например точка (960, 540)
        stiffness 800.0
        damping 0.85
        rest_length 50.0
```
...
* **`target`** (строка `"mouse"` или кортеж `(x, y)`, **обязательный**) — Целевая точка притяжения.
* **`stiffness`** (число) — Жёсткость пружины. Чем выше — тем сильнее и быстрее реагирует. По умолчанию `500.0`.
* **`damping`** (число, от 0.0 до 1.0) — Затухание колебаний. `1.0` — без затухания, `0.0` — мгновенная остановка. По умолчанию `0.9`.
* **`rest_length`** (число) — Длина покоя (расстояние, на котором сила = 0). По умолчанию `0.0`.
* **`max_force`** (число) — Ограничение максимальной силы. По умолчанию `50000.0`.
* **`max_speed`** (число) — Ограничение скорости частицы. По умолчанию `2500.0`.
* **`screen_bounds`** (логическое значение) — Удерживать частицу в пределах экрана. По умолчанию `True`.

**Параметры разброса (range):**

* `stiffness_range`, `damping_range`, `rest_length_range` — позволяют задать случайный разброс для каждого параметра при создании частицы (число или 2 числа как диапазон `[min, max]`).

## Технические особенности реализации

1. Обработчик является **аддитивным**. Это значит, что его можно сочетать с другими обработчиками, изменяющие положение частицы.

**Примеры:**

#### 1. След пружинистый за мышью
...
#### 2. Притяжение к центру экрана с колебаниями
...
Плавное блуждание частицы с случайными поворотами.
...
* **`radius`** (число) — Радиус области блуждания (максимальное отклонение от текущей траектории). По умолчанию `15.0`.
* **`speed`** (число) — Базовая скорость движения. По умолчанию `35.0`.
* **`turn_chance`** (число, от 0.0 до 1.0) — Вероятность смены направления за кадр. По умолчанию `0.35`.
* **`turn_angle`** (число, в градусах) — Максимальный угол поворота. По умолчанию `180.0`.
* **`smoothness`** (число, от 0.0 до 1.0) — Плавность поворота. Чем выше — тем мягче изменение направления. По умолчанию `0.33`.
* **`min_speed` / `max_speed`** (число) — Ограничения скорости. По умолчанию `10.0` и `100.0` соответственно.
* **`screen_bounds`** (логическое значение) — Удерживать ли в пределах экрана. По умолчанию `True`.

**Параметры разброса (range):**

* `radius_range`, `speed_range`, `turn_chance_range`, `turn_angle_range`, `smoothness_range` — случайный разброс при создании частицы (число или 2 числа как диапазон `[min, max]`).
...
## Обработчик: `rotate`
> Обработчик `rotate` использует `Transform`, что требует дополнительных вычислительных ресурсов. Это стоит учитывать при работе с большим количеством частиц.
...
* **`target_key`** (строка) – Название ключа по которому будут браться данные о скорости. Применяется, если другой обработчик хранит свои данные в своём словаре. Например, обработчик [`wander`](#обработчик-wander). Чтобы получить информацию о скорости у него вы должны задать `target_key` как `"velocity"`. А, например, обработчики `move` или `simple_move`хранят скорость как уникальный для частицы параметр, т.е. для них `target_key` задавать не нужно. По умолчанию `None`.
...
## Обработчик: `flicker`
> Обработчик `flicker` использует `Transform`, что требует дополнительных вычислительных ресурсов. Это стоит учитывать при работе с большим количеством частиц.
...
## Обработчик: `tween`
> Обработчик `tween` использует `Transform`, что требует дополнительных вычислительных ресурсов. Это стоит учитывать при работе с большим количеством частиц.
...
> Обработчик `color_curve` использует `Transform`, что требует дополнительных вычислительных ресурсов. Это стоит учитывать при работе с большим количеством частиц.
...
## Обработчики зон событий

Группа обработчиков позволяет отслеживать зоны на экране или относительно самих частиц и вызывать пользовательские функции при выполнении условий пересечения.

**Список шорткатов**

Для использования в блоках `on update/event/particle dead` доступны следующие ключевые слова:
* `zone_event` — базовый обработчик зоны (срабатывает при нахождении в зоне).
* `zone_enter` — срабатывает строго в момент **входа** частицы в зону.
* `zone_exit` — срабатывает строго в момент **выхода** частицы из зоны.
* `zone_while` — срабатывает **каждый кадр**, пока частица находится внутри зоны.

**Общие параметры обработчиков зон**

Все четыре обработчика наследуют общую конфигурацию и поддерживают следующие параметры внутри своего блока:
* **`x`** (*число*) — Координата центра зоны по оси X. По умолчанию `0.0`.
* **`y`** (*число*) — Координата центра зоны по оси Y. По умолчанию `0.0`.
* **`width`** (*число*) — Ширина зоны (или диаметр по X для окружностей/эллипсов). По умолчанию `100.0`.
* **`height`** (*число*) — Высота зоны (или диаметр по Y для эллипсов). По умолчанию `100.0`.
* **`shape`** (*строка*) — Форма зоны. Доступные варианты:
    * `"rect"` (по умолчанию) — Прямоугольник.
    * `"circle"` — Круг (в качестве радиуса используется половина `width`).
    * `"ellipse"` — Эллипс с полуосями `width/2` и `height/2`.
* **`function`** (*callable* или *список callabe*) — Функция или список функций обратного вызова. Сигнатура функции должна быть: `def my_callback(particle, context, behavior):`. По умолчанию `None`.
* **`once`** (*логическое значение*) — Флаг однократного срабатывания. Если `True`, триггер сработает только один раз за время жизни частицы (при переходе в соответствующее состояние). По умолчанию `True`.
* **`particle_local`** (*логическое значение*) — Локальный режим. Если `True`, координаты `x` и `y` рассчитываются относительно текущих координат самой частицы (`particle.x + x`), смещая зону вместе с ней. Если `False`, координаты абсолютны. По умолчанию `False`.
* **`inverse`** (*логическое значение*) — Инверсия логики. Если `True`, триггер инвертирует геометрическое условие (зона считается там, где её изначально нет). По умолчанию `False`.

**Специфика работы триггеров**

1. `zone_event` (Базовый триггер)
Проверяет нахождение частицы в указанной области. Используется для простых мгновенных проверок.
2. `zone_enter` (Вход в зону)
Срабатывает только тогда, когда на предыдущем кадре частица была вне зоны, а на текущем оказалась внутри неё.
3. `zone_exit` (Выход из зоны)
Срабатывает в момент, когда частица на предыдущем кадре находилась внутри зоны, а на текущем вышла за её пределы.
4. `zone_while` (Удержание в зоне)
Вызывает указанную функцию каждый кадр, пока частица находится внутри геометрических границ зоны (с учетом параметра `once`).

**Архитектурные особенности и изоляция данных**

1. **Хранение состояний (`particles_properties`)**:
   Каждая зона автоматически генерирует уникальный внутренний идентификатор вида `_renp_event_zone_<counter>`, под которым в свойствах частицы сохраняются её логические флаги:
   * `triggered` — сработал ли триггер (для отработки `once`).
   * `last_state` — геометрический статус частицы на предыдущем кадре (внутри/вне зоны) для вычисления дельты в `zone_enter`/`zone_exit`. 
2. **Безопасность выполнения**:
   Если параметр `function` не задан или равен `None`, обработчики возвращают статус выполнения `UpdateState.Pass`, не нагружая вычислениями игровой цикл.

**Примеры использования**

*Пример 1: Уничтожение или отметка частиц при входе в триггер*
Допустим, у нас есть функция на Python, которая как-то модифицирует частицу или регистрирует её:

```python
init python:
    def example_log_particle(particle, context, behavior):
        # Пример логики обратного вызова
        particle.destroy()
        renpy.notify("Частица: {} попала в зону {}.\nДельта: {}".format(particle, behavior, context.delta)) 
```

Тогда в DSL-системе мы можем описать круглую зону в центре экрана, при входе в которую частицы будут вызывать этот колбэк:
...
rparticles as zone_demo:
    ...
    on update:
        ...
        # Зона-триггер на вход
        zone_enter:
            x 960.0
            y 540.0
            width 300.0 # width тут используется как радиус, в данном случае радиус равен width / 2, т.е. 150
            shape "circle"
...
*Пример 2: Локальная инвертированная зона (Слежение за окружением частицы)*
Если нужно, чтобы частица реагировала, когда кто-то или что-то выходит за её собственные локальные границы:
...
rparticles as zone_demo:
    ...
    on update:
        ...
        # Вызовется, если глобальные условия изменят положение инвертированной локальной зоны
        zone_exit:
            x 0.0
            y 0.0
            width 150.0
...
## Обработчик: `mouse_event`

Обработчик событий мыши, позволяющий частицам реагировать на клики левой/правой кнопкой мыши или её перемещение, вызывая пользовательские функции. Этот обработчик работает напрямую с глобальными событиями подсистемы ввода Pygame.

```renpy
# В блоке поведения (on event)
    ...
    on event:
        mouse_event:
            mode "lmb"
...
* **`mode`** (*строка*): Режим отслеживания события мыши. Доступные варианты:
    * `"lmb"` (по умолчанию) — Нажатие левой кнопки мыши (`Left Mouse Button`).
    * `"rmb"` — Нажатие правой кнопки мыши (`Right Mouse Button`).
    * `"move"` — Перемещение курсора мыши (`Mouse Motion`).
* **`function`** (*callable* или *список callable*): Функция или список функций обратного вызова, которые выполнятся при наступлении события. Сигнатура функции должна быть: `def my_callback(context, behavior):`. По умолчанию `None`.

### Архитектура и нюансы работы

1. **Строгая контекстная зависимость**:
   Этот обработчик спроектирован для работы **исключительно** внутри блока `on event`. При попытке вызвать его внутри `on update` движок принудительно выбросит ошибку несоответствия контекста (`Invalid context type for MouseEvent: Expected: EventContext`).
2. **Безопасная нормализация**:
   Параметр `function` автоматически нормализуется при первом вызове. Если передан одиночный колбэк, он преобразуется в список; все значения `None` внутри списков автоматически фильтруются во избежание ошибки.
3. **Фильтрация валидности**:
   Если указан некорректный режим `mode` (не входящий в список разрешенных: `lmb`, `rmb`, `move`), симуляция выдаст критическую ошибку с перечислением доступных конфигураций. Если функция обратного вызова не задана, обработчик просто возвращает `UpdateState.Pass`.

### Пример использования

Регистрация функции на Python и вызов её при клике по экрану:

```python
init python:
    def spawn_spark_impulse(context, behavior):
        # context.event содержит Pygame-событие
        # behavior ссылается на экземпляр MouseEvent
        renpy.notify("Игрок кликнул мышкой в режиме: {}".format(behavior.mode))
```
...
rparticles as interactive_click:
    ...
    on event:
        # Реагируем на нажатие левой кнопки мыши
        mouse_event:
            mode "lmb"
...
Позволяет проигрывать аудиофайлы (звуковые эффекты, эмбиент или музыку) через встроенную звуковую систему Ren'Py в момент выполнения блока поведения частицы.

### Синтаксис:

```renpy
# В любом блоке поведения (например, on particle dead или on particle appear)
    ...
    on particle dead:
        sound:
...
* **`file`** (*строка*, **обязательный**): Путь к аудиофайлу в директории игры.
* **`channel`** (*строка*): Звуковой канал Ren'Py для воспроизведения. По умолчанию `"audio"`.
* **`loop`** (*логическое значение*): Зацикливать ли воспроизведение аудиофайла. По умолчанию `False`.
* **`fadein`** (*число*): Время плавного появления звука в секундах. По умолчанию `0`.
* **`fadeout`** (*число* или *None*): Время плавного затухания звука в секундах при остановке/перезаписи канала. По умолчанию `None`.
* **`volume`** (*число*): Относительная громкость воспроизведения (от `0.0` до `1.0`). По умолчанию `1.0`.

### Технические особенности

1. **Прямой вызов подсистемы Ren'Py**:
   Внутренне обработчик транслирует параметры напрямую в метод `renpy.music.play()`, полностью подчиняясь глобальным микшерам громкости игры.
2. **Мгновенное выполнение**:
   Обработчик всегда возвращает `UpdateState.Pass`. Стоит учитывать, что если вызвать `sound` в блоке `on update` без использования модификаторов ограничения (например, `oneshot`), звук начнет перезапускаться **каждый кадр**, что приведет к перегрузке аудио-канала и звуковым артефактам.

### Примеры использования

*1. Озвучка гибели частицы (Взрыв снаряда)*
Идеальный сценарий использования — одиночный вызов в блоке `on particle dead`:
...
system id "rocket_shell":
    ...
    on particle dead:
        # Проигрываем звук взрыва при уничтожении частицы
        sound:
            file "sfx/boom.wav"
            channel "audio"
            volume 0.7
...
*2. Звуковой маркер с модификатором `oneshot` внутри `on update`*
Если необходимо запустить звук в цикле обновления, но строго один раз при рождении частицы:

```renpy
on update:
    # Модификатор oneshot гарантирует однократное воспроизведение
    sound oneshot:
...
### Переопределение свойств обработчиков

Система **RenParticles** позволяет использовать существующие модели как гибкие шаблоны. Вы можете вызвать модель и "на лету" изменить параметры любого её обработчика, если у этого обработчика задан уникальный `id` (подробнее – [Идентификаторы обработчиков](#идентификаторы-обработчиков-id)).

#### Синтаксис переопределения

Чтобы переопределить свойства, добавьте двоеточие `:` после вызова модели и создайте вложенный блок. Внутри блока укажите строковый `ID` обработчика и новые значения его свойств.

```renpy
rparticles model "название_модели":
    "id_обработчика":
        параметр_1 значение
        параметр_2 значение
...
Допустим, у нас есть базовая модель следа за мышью `rparticles_mouse_trail`, где за движение отвечает обработчик с `id "move_handler"`, а за анимации – обработчик с `id "tweener"`.
...
**2. Параметризированный вызов:**
Здесь мы меняем диапазон скоростей и переопределяем блок анимации для масштабирования, не меняя саму модель в файле определений:
...
* **Наличие ID**: Переопределить можно только те обработчики (behaviors), которым при создании модели был явно присвоен `id`.
* **Дубликаты**: Система выдаст ошибку, если вы попытаетесь переопределить один и тот же `id` дважды в одном блоке.
* **Необязательность**: Вам не нужно переопределять все свойства. Если параметр не указан в блоке переопределения, будет использовано значение из базовой модели.
...
## Принудительная симуляция

`rparticles simulate` Позволяет принудительно «прогнать» систему частиц на заданное количество секунд вперёд.
...
* **`tag`** (строка) — Тег (as tag) существующей системы частиц, которую нужно симулировать. *Обязательный* параметр.
* **`время`** (положительное число) — Количество секунд, которое необходимо принудительно симулировать. *Обязательный* параметр.
* **`wait`** (флаг) — *Если указан*, игра приостановится и будет визуально проигрывать ускоренную симуляцию с микро-паузами (шагами `wait_step` или если не указан `wait_step`, то по умолчанию 0.0005 секунд).
* **`wait_step`** (положительное число) – Если указан после флага `wait`, то это будет шаг при визуальном симулировании.

**Как работает:**
- Симуляция применяется мгновенно и не требует реального ожидания, если не указан флаг wait.
- Если указан wait, Ren'Py временно входит в специальный служебный цикл, обновляя систему частиц для визуального контроля за быстрой симуляцией.
...
rparticles model "my_fire" as fire_system onlayer master

# Мгновенно симулируем 2 секунды жизни системы
rparticles simulate "fire_system" 2.0
"Система просимулирована на 2 секунды. Огонь уже разгорелся."
```

- Наглядная демонстрация с ожиданием
```renpy
rparticles simulate "fire_system" 5.0 wait
"Прошло 5 секунд симуляции с визуальными шагами."
```

- Комбинация с показом системы
Можно использовать сразу же после создания системы или первого показа модели.
```renpy
# Показываем систему
rparticles show as rparticles_test onlayer master:
    ...

# Сразу симулируем 0.5 секунды, чтобы частицы начали двигаться
rparticles simulate "rparticles_test" 0.5
```

**Нюансы:**
- Слишком большие значения время (например, 1000 секунд) без `wait` могут вызвать кратковременную "статтер" движка из-за большого количества вычислений за один кадр. Во время система никак не оптимизируется для точных расчётов.
- Если система с указанным тегом не найдена, команда `rparticles simulate` будет проигнорирована.
