...
* [Базовые классы](#базовые-классы)
    * [_Behavior](#_behavior)
    * [Типы поведений](#типы-поведений)
* [Контексты выполнения](#контексты-выполнения)
    * [RenpFContext](#renpfcontext)
    * [EventContext](#eventcontext)
* [Создание собственного поведения](#создание-собственного-поведения)
    * [Простое поведение](#простое-поведение)
    * [Поведение с состоянием на частицу](#поведение-с-состоянием-на-частицу)
* [Создание эмиттеров](#создание-эмиттеров)
    * [1. Стандартный эмиттер (Класс `Emitter`)](#1-стандартный-эмиттер-класс-emitter)
    * [2. Потоковый эмиттер (Класс `_Behavior`)](#2-потоковый-эмиттер-класс-_behavior)
* [Создание пресетов (Presets)](#создание-пресетов-presets)
    * [1. Базовая архитектура пресета](#1-базовая-архитектура-пресета)
    * [2. Пример создания пресета (`RepulsorPreset`)](#2-пример-создания-пресета-repulsorpreset)
    * [3. Специальные типы пресетов](#3-специальные-типы-пресетов)
    * [4. Полезные методы `_RFBehaviorPreset`](#4-полезные-методы-_rfbehaviorpreset)
    * [5. Рекомендации по реализации](#5-рекомендации-по-реализации)
* [Регистрация компонентов](#регистрация-компонентов)
    * [1. Статические шорткаты](#1-статические-шорткаты)
    * [2. Динамическая регистрация (Рекомендуется)](#2-динамическая-регистрация-рекомендуется)
    * [3. Технические нюансы регистрации](#3-технические-нюансы-регистрации)
    * [4. Порядок инициализации (Приоритеты)](#4-порядок-инициализации-приоритеты)
* [Работа с частицами](#работа-с-частицами)
    * [RenParticle](#rensprite)
    * [Управление визуализацией](#управление-визуализацией)
    * [Практический пример: Создание и настройка](#практический-пример-создание-и-настройка)
    * [Нюанс с `set_child`](#нюанс-с-set_child)
* [Система частиц: Ядро](#система-частиц-ядро)
...
* [Множественные системы](#множественные-системы)
    ...
    * [2. Взаимодействие между системами](#2-взаимодействие-между-системами)
* [Техническая справка: Контексты выполнения](#техническая-справка-контексты-выполнения)
* [Трансформации частиц](#трансформации-частиц)
    * [PropertyTween](#propertytween)
* [Производительность](#производительность)
    * [Оптимизация рендеринга](#оптимизация-рендеринга-стандартный-код-renpy-из-spritemanagerrender)
    * [Советы по оптимизации](#советы-по-оптимизации)
* [Отладка](#отладка)
    * [Вывод информации о системе](#вывод-информации-о-системе)
    * [Проверка состояния частиц](#проверка-состояния-частиц)
* [Расширенные паттерны](#расширенные-паттерны)
    * [Цепочки поведений](#цепочки-поведений)
    * [Условные поведения](#условные-поведения)
* [API Reference](#api-reference)
    * [Основные константы](#основные-константы)
    * [Утилиты](#утилиты)
    * [Доступ к системам](#доступ-к-системам)
* [Соглашения о коде](#соглашения-о-коде)
* [Примеры реализации](#примеры-реализации)
    * [Гравитация](#гравитация)
    * [Столкновения с границами](#столкновения-с-границами)
    * [Следование за целью](#следование-за-целью)
...
class MyEmitter(Emitter):
    ...
    def __call__(self, context):
        ...
        for i in range(self.amount):
            ...
            # Установка технических свойств спрайта (RenParticle)
            sprite.zorder = i
...
* **Изоляция**: При инициализации системы эмиттеры отделяются, которые объявлены в блоке `on update` от общего блока `on update` и помещаются в специальный приоритетный список.
...
`RenParticle` — это расширенный класс стандартного `Sprite` из движка Ren'Py. Он поддерживает отложенное применение трансформаций.
...
class RenParticle(Sprite):
    ...
    def get_transform_from_queue(self, property_key):
        """Попытка найти свойство в очереди трансформаций."""
...
Система использует **отложенную отрисовку** (Lazy Rendering). Вместо того чтобы перерисовывать `Transform` при каждом изменении `alpha` или `zoom`, `RenParticle` накапливает изменения.
...
def __call__(self, context):
    ...
    # system.create возвращает экземпляр RenParticle
...
* **Particle Pool**: Для всех систем заранее определён "пул" частиц, с которых системы берут экземпляры частиц (или убирают их обратно). По умолчанию количество таких экземпляров `2500` на все системы.
