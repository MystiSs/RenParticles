...
> **Важно:** Система автоматически вызывает `apply_transforms()` в конце цикла обновления. Она берет `_base_image` (копию оригинального спрайта) и один раз применяет к нему все накопленные свойства через `Transform`. Это значительно ресурсы процессора.
