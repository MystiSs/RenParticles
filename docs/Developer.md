...
Для документации разработчика важно не только показать код, но и четко разграничить архитектурные нюансы, которые влияют на производительность и логику.

Вот переработанный вариант описания эмиттеров, структурированный для быстрого понимания.
...
class RepulsorPreset(_RFBehaviorPreset):
    ...
    def distribute_properties(self):
        # Всегда вызываем super для проверки @required_field
...
> **Важно:** Крайне не рекомендуется изменять этот словарь напрямую в исходных файлах движка. Это затруднит обновление ядра RenParticles в будущем.
...
* **Валидация типов**: Функции `add_shortcut` и `add_preset` выбрасывают `TypeError`, если регистрируемый класс не является наследником соответствующих базовых классов (`_Behavior` или `_RFDynamicBehaviorPreset`).
...
* **Приоритеты**: При поиске компонента парсер DSL сначала проверяет статические шорткаты, а затем динамические. Это позволяет "перекрывать" системные поведения вашими собственными, если это необходимо.
...
`RenSprite` — это расширенный класс стандартного `Sprite` из движка Ren'Py. Он оптимизирован для работы в высокопроизводительных системах и поддерживает отложенное применение трансформаций.
...
* **`x, y`** (float): Координаты центра частицы на экране.
* **`zorder`** (int): Порядок отрисовки (чем выше, тем ближе к игроку).
* **`live`** (bool): Статус частицы. Если `False`, система удалит её в следующем кадре.
* **`lifetime`** (float): Оставшееся время жизни в секундах.
* **`lifetime_max`** (float): Оставшееся время жизни в секундах. Базовые обработчики не изменяют этот атрибут. Не рекомендуется его изменять в принципе.
* **`events`** (bool): Флаг, разрешающий частице реагировать на события (по умолчанию `False` для оптимизации).
...
2. **`queue_transform_additive(**props)`**: Складывает значения. Позволяет нескольким независимым поведениям одновременно влиять на одно свойство (например, два разных осциллятора, влияющих на `xpos`).

> **Важно:** Система автоматически вызывает `apply_transforms()` в конце цикла обновления. Она берет `_base_image` (копию оригинального спрайта) и один раз применяет к нему все накопленные свойства через `Transform`. Это значительно экономит ресурсы процессора.
...
* `reset()`: Полная очистка системы. Удаляет все частицы и очищает их свойства в `particles_properties`, возвращая систему в исходное состояние.
